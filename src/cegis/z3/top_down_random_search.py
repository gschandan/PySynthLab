...
from z3 import *
from z3 import BoolSortRef, ArithSortRef, BitVecSortRef, ArraySortRef, DatatypeSortRef, FiniteDomainSortRef, FPSortRef, \
    FPRMSortRef, ReSortRef, SeqSortRef, CharSortRef, TypeVarRef, SortRef

from src.cegis.z3.synthesis_problem import SynthesisProblem
from src.cegis.z3.synthesis_strategy import SynthesisStrategy


class TopDownEnumerativeSynthesis(SynthesisStrategy):
    """
    Top down enumerative strategy - starting with the function to synthesise and build expressions using grammar rules
    """
    ...
    def fast_enum(self, sort: z3.SortRef, size: int) -> List[z3.ExprRef | bool]:
        """Generates expressions for the given function up to the specified maximum depth."""
        if current_depth > max_depth:
            return []  # Base case: exceeded depth limit

        if (func_name, max_depth) in self.term_cache:
            return self.term_cache[(func_name, max_depth)]

        terms = []
        unique_terms = set()
        output_sort, arg_sorts = self.grammar[func_name]
        arg_names = list(self.problem.context.z3_synth_function_args[func_name].keys())
        args = [self.problem.create_z3_variable(name, sort) for name, sort in zip(arg_names, arg_sorts)]
        ...
        # Recursive Case
        for constructor, constructor_arg_sorts in self.grammar.get(output_sort, []):
            if constructor == func_name: # Avoid recursion
                continue
            for arg_terms in product(*[self.generate_expressions(func_name, max_depth, current_depth + 1)
                                       for _ in constructor_arg_sorts]):
                if constructor == "Ite":
                    # For Ite, the first argument must be a Bool
                    if not isinstance(arg_terms[0], z3.BoolRef):  # Handle cases where the first argument may be an Int
                        continue
        ...
        self.term_cache[(func_name, max_depth)] = terms
        return terms
